...
namespace CudaShader
{
    ...
    __device__ static void WireframeShaderCUDA(shaderParams p)
    {
        ...
        // Set output color
        p.out_color[colorIdx + 0] = 1 - opacity;
        p.out_color[colorIdx + 1] = 1 - opacity;
        p.out_color[colorIdx + 2] = 1 - opacity;
    }

    ///// Assign all the shaders to their short handles.
    shader outlineShader = &OutlineShaderCUDA<NUM_CHANNELS>;
    shader wireframeShader = &WireframeShaderCUDA<NUM_CHANNELS>;
