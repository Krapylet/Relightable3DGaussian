...
def render_view(viewpoint_camera: Camera, pc: GaussianModel, pipe, bg_color: torch.Tensor,
                scaling_modifier=1.0, override_color=None, is_training=False, dict_params=None, time:float=0.0, dt:float=0.0, shaderTextureMaps_ptr=None):
    ...
    raster_settings = GaussianRasterizationSettings(
        ...
        time = time,
        dt = dt,
        shaderTextureMaps_ptr = shaderTextureMaps_ptr
    )
...
def render_neilf(viewpoint_camera: Camera, pc: GaussianModel, pipe, bg_color: torch.Tensor,
                 ...
                 is_training=False, dict_params=None, time=0.0, dt=0.0, shaderTextureMaps_ptr=None):
    """
    Render the scene.
    Background tensor (bg_color) must be on GPU!
    """
    results = render_view(viewpoint_camera, pc, pipe, bg_color,
                          scaling_modifier, override_color, is_training, dict_params, time, dt, shaderTextureMaps_ptr)
