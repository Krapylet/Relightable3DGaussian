...
namespace PostProcess 
{
    __device__ PostProcessShaderParams::PostProcessShaderParams(PackedPostProcessShaderParams p, int x, int y, int pixCount):
        ...
        out_color(p.out_color),                
        out_shader_color(p.out_shader_color),  
        ...
        //// Scene textures:
        out_opacity(p.out_opacity),        // Transparrency mask for all rendered objects in the scene.
        ...
        // output
        out_postprocess_color((glm::vec3 *const)p.out_postprocess_color + pixel_idx)
    {
